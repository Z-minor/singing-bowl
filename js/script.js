...
function 新個體變速(主體){
    ...
    //目標速度(理想速度) =每幀前進剩餘路程的1/x 但是會受最高速度影響(?)
    //理想每幀速度變化幅度 = (理想速度-真實速度)/100(?) 但是會受最高加速程度影響
    //真實每幀速度變化幅度 由加加速度控制 必須是連續的

    let 目標點 = window.scrollY;
    //let 理想速度 = (目標點-主體.現在位置)/主體.每一影格前進等分;
    let 理想速度 = (目標點-主體.現在位置)/10;
    ...
    let 理想每幀速度變化幅度 = (理想速度-主體.現在速度)/主體.每一影格前進等分;
    //理想每幀速度變化幅度 = (理想速度-主體.現在速度)/((主體.每一影格前進等分-1)*((100-主體.加速程度)*0.01)+1);
    ...
    if(理想每幀速度變化幅度<0 && 主體.現在位置<目標點 || 理想每幀速度變化幅度>0 && 主體.現在位置>目標點){
        主體.加速程度+=10;
        if(主體.加速程度>100){
            主體.加速程度=100;
        }
    ...
    /*if(Math.abs(理想每幀速度變化幅度)>主體.最高加速程度){
        if(理想每幀速度變化幅度>0){
            理想每幀速度變化幅度 = 主體.最高加速程度;
        }
        else{
            理想每幀速度變化幅度 = -主體.最高加速程度;
        }
...
let 個體陣列= [];
let 個體參數= [[true,"位子1","y","3000px","50vh",0.3,0,0,0.1,0,100,0.05,0.9,false,[0.3,0.5],[0.8,0.5],0],[true,"位子2","y","0px","50vh",移動範圍為整個視窗大小的係數,0,0,5,0,10,0.005,0.6],
